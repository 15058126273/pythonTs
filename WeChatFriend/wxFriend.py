...
def main():
    ...
    imageIndex = 0
    needDown = False
    down = input("是否下载头像？输入y下载，输入其他任意值则不下载")
    if down == 'y' or down == 'Y':
        needDown = True
        downPath = input("请输入你想将头像保存到哪个路径（如：E:/headImg/）：")
        while not os.path.exists(downPath):
            downPath = input("输入的路径不存在，请重新输入：")
    for Member in MemberList:
        imageIndex = imageIndex + 1
        if needDown:
            name = downPath+str(imageIndex)+'_'+str(math.floor(time.time()))+'.jpg'
        ...
        name = name[0:21] + "..." if len(name) > 20 else name
        sign = str.strip(Member['Signature'])

        sign = "".join(sign.split())
        sign = 'nosign' if sign == '' else sign
        sign = sign[0:21] + "..." if len(sign) > 20 else sign

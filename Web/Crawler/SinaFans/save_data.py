...
def save(video, update):
    ...
    try:
        ...
        coin_num = video.get('coin_num')
        share_num = video.get('share_num')
        if update:
            sql = 'update bi_video set CHANGE where id = '+str(video_id)
            change = ''
            value = []
            if link is not None:
                change += 'link=%s,'
            ...
            if coin_num is not None:
                change += 'coin_num=%s,'
                value.append(coin_num)
            if share_num is not None:
                change += 'share_num=%s,'
                value.append(share_num)
            if change.endswith(','):
                change = change[:-1]
            sql = sql.replace('CHANGE', change)
            cur.execute(sql, tuple(value))
        else:
            sql = 'insert into bi_video (COLUMN) values (VALUES)'
            column = ''
            values = ''
            value = []
            ...
            if coin_num is not None:
                column += ',coin_num'
                values += ',%s'
                value.append(str(coin_num))
            if share_num is not None:
                column += ',share_num'
                values += ',%s'
                value.append(str(share_num))
